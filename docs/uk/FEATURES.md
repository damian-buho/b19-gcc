...
- Обгортки компілятора в `/usr/local/lib/compile-cache/` прозоро перехоплюють виклики `gcc`, `g++`, `cc` і `c++` за пріоритетом PATH — жодних змін у засобах збирання не потрібно.
