...
- Вісім типових перевірок у базовому образі: місце на диску, доступність файлової системи для запису та TCP-перевірка прослуховування виконуються завжди; HTTPS-з’єднуваність, DNS-резолв і TCP-доступність виконуються лише там, де `B19_HEALTH_EGRESS=true`, тож контейнер, який ніколи не звертається до інтернету, не має жодної перевірки, яку може завалити третя сторона.
- Перевірки зовнішнього доступу толерантні до збоїв — успіх на будь-якій цілі зараховується як прохід.
- Усі перевірки зовнішнього доступу автоматично пропускаються в режимі offgrid; усі перевірки можна вимкнути під час виконання.
